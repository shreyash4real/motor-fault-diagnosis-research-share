"""
STEP 5 (v2) — STFT CNN TRAINING, configurable for multiple runs
================================================================
Supports running multiple experiments from one script by editing the
CONFIG block at the top, then re-running. Each run writes outputs to
its own subfolder (training/{RUN_NAME}/) so nothing gets overwritten.

PLANNED RUNS (from diagnosis of v1 results)
-------------------------------------------
  Run A (baseline on new splits):
      FEATURES_DIR = "...\\v2_speed_strat\\features"
      HEALTHY_SUBSAMPLE_N = None
      USE_CLASS_WEIGHTS = False
      RUN_NAME = "A_v2_baseline"

  Run B (new splits + balanced):
      FEATURES_DIR = "...\\v2_speed_strat\\features"
      HEALTHY_SUBSAMPLE_N = 855
      USE_CLASS_WEIGHTS = False
      RUN_NAME = "B_v2_healthy_sub"

  Run C (v1 splits + balanced — isolates subsampling effect):
      FEATURES_DIR = "...\\features"      (original v1 features)
      HEALTHY_SUBSAMPLE_N = 855
      USE_CLASS_WEIGHTS = False
      RUN_NAME = "C_v1_healthy_sub"

Compare A vs C to isolate split effect.
Compare A vs B to isolate subsampling effect on the fixed splits.
...
# ═══════════════════════════════════════════════════════════════════════════
#  RUN CONFIG — edit these between runs
# ═══════════════════════════════════════════════════════════════════════════

RUN_NAME     = "VIB_A_baseline"
FEATURES_DIR = legacy_path(r"C:\Project Work\outputs\vibration\v2_speed_strat\features")
OUTPUT_BASE  = legacy_path(r"C:\Project Work\outputs\vibration\training")

# Healthy subsampling — None to disable, int to subsample to that size
HEALTHY_SUBSAMPLE_N = None   # try 855 for balanced run
HEALTHY_CLASS_NAME  = "healthy 1"

# Class weighting — True for inverse-frequency weighted CE
USE_CLASS_WEIGHTS = False
...
DROPOUT_P2      = 0.3
WEIGHT_DECAY    = 1e-4
LABEL_SMOOTHING = 0.0        # try 0.1 if still overfitting

# Core hyperparams (usually don't change between runs)
EPOCHS        = 1
BATCH_SIZE    = 16
LEARNING_RATE = 1e-3
PATIENCE      = 8
...
def load_pt_dataset(name: str, features_dir: Path,
    ...
    """Load one .pt file. Optionally subsample the healthy class (train only)."""
    ...
    if subsample_healthy_n is not None and HEALTHY_CLASS_NAME in class_names:
        ...
        if n_healthy > subsample_healthy_n:
            ...
            # Materializes from mmap but the train set is still small enough to fit
...
class STFTCNN(nn.Module):
    """5-block VGG-style encoder: 12 -> 24 -> 48 -> 96 -> 96. ~340k params."""
    def __init__(self, num_classes: int = 4, feature_dim: int = 96,
        ...
        self.b3 = block(24,  48)
        self.b4 = block(48,  96)
        self.b5 = block(96,  feature_dim, pool=False)
...
def main():
    ...
    # ── Model ───────────────────────────────────────────────────────────────
    model = STFTCNN(num_classes=num_classes,
                    dropout1=DROPOUT_P1, dropout2=DROPOUT_P2).to(device)
    n_params    = sum(p.numel() for p in model.parameters())
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel: STFTCNN 12-24-48-96-96")
    print(f"  total parameters    : {n_params:,}")
    print(f"  trainable parameters: {n_trainable:,}")
    print(f"  params per train sample: {n_params / len(train_ds):.1f}")

    # Criterion — weighted CE if requested, with optional label smoothing
    ...
    config_lines = [
        ...
        f"model             : STFTCNN 12-24-48-96-96",
