...
FEATURES_DIR   = legacy_path(r"C:\Project Work\Outputs\4class\v2_speed_strat\mel\features")
IMAGES_DIR     = legacy_path(r"C:\Project Work\Outputs\4class\v2_speed_strat\mel\stft_images")
...
# Mel filterbank — built once, reused for every segment.
# Maps 513 linear STFT bins -> N_MEL_BINS mel bins across 0..FREQ_CUTOFF_HZ.
# n_fft must match NPERSEG. n_fft//2 + 1 = 513 linear bins.
_MEL_FB = librosa.filters.mel(
    sr=FS, n_fft=NPERSEG, n_mels=N_MEL_BINS,
    fmin=0.0, fmax=FREQ_CUTOFF_HZ, htk=False, norm="slaney",
).astype("float32")
...
def compute_spectrogram_db(sig_3phase: np.ndarray) -> np.ndarray:
    """
    (3, 20000) signal -> (3, N_MEL_BINS, 149) float32 log-magnitude dB.
    Mel-scaled: linear STFT -> mel filterbank -> log. The filterbank
    integrates power (not magnitude), so we square the magnitude first,
    apply the filterbank, then take 10*log10 for dB.
    """
    spec_phases = []
    for ph in range(3):
        _, _, Zxx = stft(sig_3phase[ph], fs=FS, window="hann",
                         nperseg=NPERSEG, noverlap=NOVERLAP,
                         boundary=None, padded=False)
        power = (np.abs(Zxx) ** 2).astype(np.float32)     # (513, 149)
        mel_power = _MEL_FB @ power                        # (N_MEL_BINS, 149)
        log_mel = 10.0 * np.log10(mel_power + 1e-10)
        spec_phases.append(log_mel)
...
def main():
    ...
    print(f"  mel filterbank: {N_MEL_BINS} bins over 0..{FREQ_CUTOFF_HZ} Hz (Slaney norm)")
    print(f"  expected sample shape: (3, {N_MEL_BINS}, 149) float32")
