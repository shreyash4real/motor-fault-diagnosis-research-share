...
# ─── CONFIGURE ────────────────────────────────────────────────────────────────
RAW_ROOT      = legacy_path(r"C:\Project Work\Dataset\Vibration\Motor-2")
DENOISED_ROOT = legacy_path(r"C:\Project Work\Denoised\Vibration\Motor-2")
SPLITS_CSV    = legacy_path(r"C:\Project Work\outputs\vibration\v2_speed_strat\splits.csv")
FEATURES_DIR  = legacy_path(r"C:\Project Work\outputs\vibration\v2_speed_strat\features")
IMAGES_DIR    = legacy_path(r"C:\Project Work\outputs\vibration\v2_speed_strat\stft_images")
SAVE_PNGS      = True
# ──────────────────────────────────────────────────────────────────────────────
...
FREQ_CUTOFF_HZ  = 6000
NUM_CHANNELS = 5
def load_full_all_channels(ch1_path: str, col_idx: int) -> np.ndarray:
    """
    Load one full 12-second column across all 5 accelerometers.
    Returns (5, 240_000) float32. Each of ch1..ch5 is read exactly once.
    """
    channels = []
    for ch in ("ch1", "ch2", "ch3", "ch4", "ch5"):
        path = ch1_path.replace("-ch1.csv", f"-{ch}.csv")
        df = pd.read_csv(path)
        channels.append(df.iloc[:, col_idx + 1].values.astype(np.float32))
...
PNG_VMIN_DB, PNG_VMAX_DB = -80.0, 0.0
PNG_CMAP = "jet"
...
def compute_spectrogram_db(sig: np.ndarray) -> np.ndarray:
    """(C, 20000) signal -> (C, F, 149) float32 log-magnitude dB spectrogram."""
    spec_channels = []
    for ch_i in range(sig.shape[0]):
        f, _, Zxx = stft(sig[ch_i], fs=FS, window="hann",
                         nperseg=NPERSEG, noverlap=NOVERLAP,
                         boundary=None, padded=False)
        mag = np.abs(Zxx).astype(np.float32)
        log_mag = 20.0 * np.log10(mag + 1e-10)
        log_mag = log_mag[f <= FREQ_CUTOFF_HZ]
        spec_channels.append(log_mag)
    return np.stack(spec_channels).astype(np.float32)
...
def save_channel_pngs(spec_db: np.ndarray, out_dir: Path,
    ...
    for ch_i in range(spec_db.shape[0]):
        img = spec_db[ch_i]
...
def process_split(splits_df: pd.DataFrame, split: str,
    ...
    if spec_shape is None:
        ...
        full = load_full_all_channels(*first_key)
    ...
    if save_png_flag:
        ...
        print(f"   5 PNGs per segment → {n_samples * 5:,} files total")
    print()
    ...
    for col_i, ((ch1_path, col_idx), entries) in enumerate(groups.items(), 1):
        try:
            full_signal = load_full_all_channels(ch1_path, col_idx)
        ...
        stride = (full_signal.shape[1] - WINDOW_SAMPLES) // (n_segs - 1) if n_segs > 1 else 0

        for entry in entries:
            start = entry["seg_idx"] * stride
            sig   = full_signal[:, start:start + WINDOW_SAMPLES]
...
def main():
    ...
    print(f"  freq_cutoff={FREQ_CUTOFF_HZ} Hz → ~308 freq bins")
    print(f"  expected sample shape: (5, 308, 149) float32")
    print(f"  .pt normalization: per-image z-score (across 5 channels)")
    print()
    print("PNG CONVENTIONS:")
    print(f"  filename : colXX_segYYY_ch{{1..5}}.png  (no labels)")
    print(f"  colormap : {PNG_CMAP}")
    print(f"  range    : [{PNG_VMIN_DB}, {PNG_VMAX_DB}] dB  (raw log-magnitude)")
    print(f"  size     : native (~308 × 149 pixels)")
