...
RUN_NAME   = "E5_mod_resnet"    # or "vgg_style"

# Only used when MODEL_TYPE == "vgg_style"
CHANNELS = (12, 24, 48, 96, 96)
...
DROPOUT_P1      = 0.3   # single dropout before the final Linear in ModifiedAlexNet
DROPOUT_P2      = 0.3
WEIGHT_DECAY    = 1e-4
LABEL_SMOOTHING = 0.0
...
class ModifiedAlexNet(nn.Module):
    ...
    def __init__(self, num_classes: int = 4, dropout: float = 0.5):
        ...
        # Conv stack — filter sizes and strides exactly as AlexNet / paper.
        ...
        # GAP replaces the three FC layers
        self.gap   = nn.AdaptiveAvgPool2d(1)
...
def build_model(model_type: str, num_classes: int, channels: tuple,
    ...
    elif model_type == "modified_resnet":
        return ModifiedResNet(num_classes=num_classes,
                         dropout=dropout1,
                         base_channels=32)
    else:
...
# ──────────────────────────────────────────────────────────────────────────────
#  Model 3 — Modified ResNet (STFT-aware, Option B)
# ──────────────────────────────────────────────────────────────────────────────
...
class ModifiedResNet(nn.Module):
    """
    STFT-aware ResNet:
      - No initial downsampling
      - Gradual downsampling in deeper layers
      - GAP + Linear head (same philosophy as Modified AlexNet)
      - Designed for ~3M params with base_channels=32
    """

    def __init__(self, num_classes=4, dropout=0.3, base_channels=32):
        super().__init__()

        self.in_planes = base_channels

        # Stem (NO aggressive downsampling)
        self.conv1 = nn.Conv2d(3, base_channels, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1   = nn.BatchNorm2d(base_channels)

        # ResNet-18 structure (2,2,2,2)
        self.layer1 = self._make_layer(base_channels,     2, stride=1)  # preserve resolution
