"""
ENSEMBLE EVALUATION — ROBUST STFT + DWT + ENVELOPE (nobpfo100)
==============================================================
Evaluates the final robust ensemble for the nobpfo100 perturbation experiment.
...
MEMBERS:
  ...
  3. ENVELOPE: ENVELOPE_resnet_v3_reg (1D ResNet v3 with heavy regularization)

STRATEGY:
  Temperature-calibrated equal-weight soft voting. 
  Loads precomputed test and val logits exported natively by the training scripts.
"""
...
ENSEMBLE_NAME = "ENSEMBLE_robust_stft_dwt_envelope_v3_temperature"

MEMBERS = {
    "stft_robust": "STFT_robust_resnet",
    "dwt":         "DWT_multibranch_plain_v1",
    "envelope_v3": "ENVELOPE_resnet_v3_reg"
}
...
def load_split_logits(run_name: str, split: str) -> tuple[np.ndarray, np.ndarray, list[tuple]]:
    ...
    if not (lg_path.exists() and lb_path.exists() and sg_path.exists()):
        raise FileNotFoundError(f"Missing {split} outputs in {run_dir}. Did training complete?")
...
def main():
    print("=" * 70)
    print("ROBUST ENSEMBLE EVALUATION (nobpfo100)")
    print("=" * 70)
