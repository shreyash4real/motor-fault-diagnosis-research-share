...
RUN_NAME   = "DWT_x4_passband_shared"
...
def load_pt_dataset(name: str, features_dir: Path,
    ...
    features_per_level = features_per_level[:KEEP_BAND_COUNT]
    level_names        = level_names[:KEEP_BAND_COUNT]
    level_bands_hz     = level_bands_hz[:KEEP_BAND_COUNT]
    level_lengths      = level_lengths[:KEEP_BAND_COUNT]
...
def stem_config(length: int) -> tuple[int, int]:
    """Shared stem config — same kernel/stride for every sub-band (DWT-X4)."""
    return STEM_KERNEL, STEM_STRIDE
