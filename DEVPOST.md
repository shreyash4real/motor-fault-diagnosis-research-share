# CurrentGuard — Devpost submission kit

## One-line pitch

CurrentGuard helps maintenance teams read induction-motor health from the three-phase current they can already measure.
...
I am an electrical engineer, so I started with the practical measurement question: what can a plant capture cheaply and consistently? Three-phase motor current is already part of the system. CurrentGuard turns a short current recording into three complementary diagnostic views—STFT, DWT, and envelope spectrum—then combines specialized classifiers to distinguish a healthy baseline from stator winding, bearing, and broken-rotor-bar fault patterns.
...
2. **0:25–0:55 — Product.** Open CurrentGuard. Load the verified reference case and explain that a production pilot would accept a plant current recording; the public demo uses committed evidence so its claims are inspectable.
